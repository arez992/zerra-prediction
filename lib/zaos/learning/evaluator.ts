...
function evaluatePaymentAudit(
  ...
  const completed =
    getNumber(data.completed) ?? 0;

  const failed =
    getNumber(data.failed) ?? 0;

  const unknown =
    getNumber(data.unknown) ?? 0;

  const successRate =
    getNumber(data.successRate);
  ...
  if (
    ...
    notes.push(
      input.executionMessage ||
        "Payment audit did not complete successfully."
    );
  ...
  if (
    ...
    notes.push(
      "The audit completed, but payment health still needs improvement."
    );
  ...
  notes.push(
    "The audit completed, but payment failure levels remain high."
  );
